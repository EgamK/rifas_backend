...
export const createPurchase = async (req: Request, res: Response) => {
  ...
  const { name, dni, phone, email, quantity, operationNumber, codRef } = req.body; // 👈 añadimos codRef
  ...
  try {
    ...
    const [purchase] = await Purchase.create([{
      ...
      status: "PENDING",
      operationNumber,
      codRef:"SCR" // 👈 guardamos referencia en la compra
    }], { session });
...
export const listPurchases = async (_req: Request, res: Response) => {
  try {
    const purchases = await Purchase.find()
      .populate("raffleId", "title") // 👈 solo traemos el campo title
      .lean();

    // Adaptar la respuesta para que tenga raffleTitle en vez de raffleId
